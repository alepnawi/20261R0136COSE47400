#!/usr/bin/env python3
"""Run the low-latency binary switch-variant experiment."""
...
EXPERIMENT_NAME = "exp6_switch_variants"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run the low-latency binary switch-variant experiment.")
    ...
    parser.add_argument("--exp6-guarded-top-k", type=int, default=2)
    parser.add_argument("--exp6-sticky-hold-steps", type=int, default=1)
    parser.add_argument("--exp6-margin-threshold", type=float, default=0.5)
...
def main():
    ...
    for row in selected_rows:
        ...
        print(
            {
                ...
                "decoder_names": list(EXP6_DECODER_NAMES),
            }
        )
        if args.dry_run:
            continue
        ...
        evaluator = ExperimentEvaluator(evaluator_args, EXP6_DECODER_NAMES)
        metadata = {
            ...
            "decoder_names": list(EXP6_DECODER_NAMES),
            "hypothesis": (
                "single_pass_switch_rules_can_recover_useful_contrast_corrections_without_block_latency"
            ),
            "experiment_note": (
                "pure_vs_guarded_vs_sticky_vs_margin_binary_switch_rules_under_greedy_vs_contrast_views"
            ),
            ...
            "guarded_top_k": evaluator_args.exp6_guarded_top_k,
            "sticky_hold_steps": evaluator_args.exp6_sticky_hold_steps,
            "margin_threshold": evaluator_args.exp6_margin_threshold,
        }
