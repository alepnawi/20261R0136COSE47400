#!/usr/bin/env python3
"""Run exp13 as a matched factorial ablation for score space vs relative-top filtering."""
...
EXPERIMENT_NAME = "exp13_logit_filter_factorial"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Run exp13 as a matched update1 factorial study for log-prob vs raw-logit contrast "
            "and relative-top filtering on vs off."
        )
    )
...
def main():
    ...
    for row in selected_rows:
        ...
        print(
            {
                ...
                "decoder_names": list(EXP13_DECODER_NAMES),
            }
        )
        if args.dry_run:
            continue
        ...
        evaluator = ExperimentEvaluator(evaluator_args, EXP13_DECODER_NAMES)
        metadata = {
            ...
            "decoder_names": list(EXP13_DECODER_NAMES),
            "hypothesis": (
                "within_a_matched_update1_family_the_gain_over_official_dola_can_be_decomposed_into_"
                "a_score_space_effect_logprob_vs_rawlogit_a_relative_top_filter_effect_on_vs_off_"
                "and_a_possible_interaction_between_them"
            ),
            "experiment_note": (
                "matched_update1_factorial_comparison_for_logprob_vs_rawlogit_and_relative_top_on_vs_off"
            ),
            "factorial_axes": {
                "score_space": ["logprob", "raw-logit"],
                "relative_top_filter": ["on", "off"],
            },
            "matched_controls": {
                "selected_layer_refresh": "every_token_update1",
                "selected_layer_rule": "step_local_jsd_argmax_same_as_official_dola",
            },
            "relative_top_threshold": evaluator_args.dola_relative_top,
            "relative_top_mask_value": evaluator_args.dola_relative_top_value,
        }
